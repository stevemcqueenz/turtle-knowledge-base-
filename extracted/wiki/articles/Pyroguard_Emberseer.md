---
title: "Pyroguard Emberseer"
url: "https://turtle-wow.fandom.com/wiki/Pyroguard_Emberseer"
source_type: "community wiki (turtle-wow.fandom.com), not an official source"
revision: "2025-12-21T16:33:46Z"
fetched: "2026-09-10T07:39:26Z"
is_turtle_content: false
patch: null
categories: ["Black dragonspawn", "Blackrock Spire mobs", "Bosses"]
---
# Pyroguard Emberseer

| field | value |
|---|---|
| name | Solakar Flamewreath |
| image | Solakar.jpg |
| level | 60 |
| type | Elite |
| race | Dragonspawn |
| creature | Dragonkin |
| location | Rookery, Upper Blackrock Spire |
| instance | Upper Blackrock Spire |
| faction | Combat |
| status | Killable |

**Solakar Flamewreath** is spawned at the end of the "Father Flame" event in the [Upper Blackrock Spire](Upper Blackrock Spire) [Rookery](Rookery).  The "Father Flame" event can be trouble for the unprepared and many groups choose to skip it. The event that spawns him begins by looting the pillar in the left side of the Rookery, surrounded by eggs and elite dragonkin.

## Dungeon Journal
*Gaining favor within the black dragonflight is a mixed blessing. Solakar Flamewreath is entrusted with the care of Nefarian's dragonspawn, but he has been warned he will pay a terrible price for any whelp that dies under his supervision--or simply fails to live up to expectations.*

## Ability
## Strategy
The Rookery is full of [whelp](whelp) eggs that hatch if a player gets too close. While the whelps are easily defeated in smaller numbers, they can quickly overwhelm a group if not attended to promptly. Great care should be taken at all times to avoid hatching the eggs. Note that trying to clear the eggs beforehand is futile, as they have a fast respawn timer.

Begin by clearing out the few [dragonspawn](dragonspawn) that occupy the Rookery as you enter. For this event the raid should be positioned as one large group inside the Rookery. This is to minimize [LoS](LoS) problems and to maximize the effectiveness of [AoE](AoE) attacks.

One character must activate the pillar in the far end of the room. It is practically impossible to approach the pillar without going near a few eggs and they will hatch. You can either kill these whelps or use a character capable of [stealth](stealth). Even though stealth won't prevent the eggs from hatching, the stealthed character won't get [aggro](aggro) from the hatched whelps and after a short while they will despawn.

When the room is clear and the group positioned, the character at the pillar can loot it and start the event proper. The event spawns five waves of 2 dragonkin [mob](mob)s ending with Solakar Flamewreath. The waves of dragonkin are combinations of [Rookery Guardian](Rookery Guardian)s and [Rookery Hatcher](Rookery Hatcher)s. It is very important that all [DPS](DPS) is focused on the Rookery Hatchers as soon as possible. The hatchers move from egg to egg hatching them and adding whelps to the battle. The sooner they are killed the easier the encounter.

Solakar Flamewreath himself is a straightforward encounter and is relatively easy.

## Loot
## External links
