---
title: "Razorfen Kraul"
url: "https://turtle-wow.fandom.com/wiki/Razorfen_Kraul"
source_type: "community wiki (turtle-wow.fandom.com), not an official source"
revision: "2026-05-11T08:04:14Z"
fetched: "2026-09-10T07:39:28Z"
is_turtle_content: true
patch: "1.18.0"
categories: ["Barrens subzones", "Caves", "Dungeons", "Quilboar territories", "Razorfen Kraul", "Turtle WoW content"]
---
# Razorfen Kraul

| field | value |
|---|---|
| name | Razorfen Kraul |
| aka | RFK |
| ss | Razorfen Kraul loading screen.jpg |
| caption | Razorfen Kraul loading graphic |
| loc | The Barrens |
| race | Quilboar |
| boss | Charlga Razorflank |
| type | Dungeon |
| level | 29-38 |
| players | 5 |
| bosses |  |

-Razorfen Kraul** is the ancestral home of the [quilboar](quilboar), heavily entrenched within the thorns that supposedly sprang from the corpse of the demigod Agamaggan. It is found in the southern [Barrens](Barrens), at the far southern edge, west side.

## History
From the [World Dungeons](https://wowpedia.fandom.com/wiki/Game_Guide/World_Dungeons) page on the official World of Warcraft Community Site:
  *Ten thousand years ago - during the [War of the Ancients](War of the Ancients), the mighty [demigod](demigod), [Agamaggan](Agamaggan), came forth to battle the [Burning Legion](Burning Legion). Though the colossal boar fell in combat, his actions helped save [Azeroth](Azeroth) from ruin. Yet over time, in the areas where his blood fell, massive thorn-ridden vines sprouted from the earth. The [quilboar](quilboar) - believed to be the mortal offspring of the mighty god, came to occupy these regions and hold them sacred. The heart of these thorn-colonies was known as the [Razorfen](Razorfen). The great mass of Razorfen Kraul was conquered by the old crone, Charlga Razorflank. Under her rule, the shamanistic quilboar stage attacks on rival tribes as well as [Horde](Horde) villages. Some speculate that Charlga has even been negotiating with agents of the Scourge - aligning her unsuspecting tribe with the ranks of the Undead for some insidious purpose.*

## Geography
### Maps
<gallery>
WorldMap-RazorfenKraul.jpg
</gallery>

## Dungeon denizens
{|class="fandom-table"
!Bosses
!Monsters
!NPCs
|-valign=top
|
-   (Final Boss)
-  
-  
-  
-  
-  
-  
-   (Rare)
-   (Rare)
|
  -Oozes**
-  [Blood of Agamaggan](Blood of Agamaggan)
  -Bats**
-  [Kraul Bat](Kraul Bat)
-  [Greater Kraul Bat](Greater Kraul Bat)
  -Boars**
-  
-  
-  
  -Razorfen Tribe**
-  [Razorfen Servitor](Razorfen Servitor)
-  [Razorfen Warrior](Razorfen Warrior)
-  [Razorfen Quilguard](Razorfen Quilguard)
-  [Razorfen Warden](Razorfen Warden)
-  [Razorfen Handler](Razorfen Handler)
  -  [Tamed Hyena](Tamed Hyena)
-  [Razorfen Geomancer](Razorfen Geomancer)
  -  [Stone Rumbler](Stone Rumbler)
-  [Razorfen Defender](Razorfen Defender)
-  [Razorfen Groundshaker](Razorfen Groundshaker)
-  [Razorfen Beast Trainer](Razorfen Beast Trainer)
  -  [Tamed Battleboar](Tamed Battleboar)
-  [Razorfen Stalker](Razorfen Stalker)
-  [Razorfen Dustweaver](Razorfen Dustweaver)
  -  [Wind Howler](Wind Howler)
-  [Razorfen Totemic](Razorfen Totemic)
-  [Quilguard Champion](Quilguard Champion)
-  [Razorfen Beastmaster](Razorfen Beastmaster)
  -  [Tamed Battleboar](Tamed Battleboar)
-  [Razorfen Spearhide](Razorfen Spearhide)
-  [Ward Guardian](Ward Guardian)
  -Death's Head Cult**
-  
-  
-  
-  
-  
-  
  -Bramblehide**
-  
-  
|
-  
-  
|-valign=top
|}

## Quest guide
{| class="fandom-table"
|-
! Quest name
! Quest giver
! Zone location
|-
!colspan="3"|  Alliance quests
|- class="alt"
| [quest: The Gnarled Bramblehide]
| 
| [Thalanaar](Thalanaar), [Feralas](Feralas)
|-
| [quest: The Crone of the Kraul]
| 
| [Thalanaar](Thalanaar), [Feralas](Feralas)
|-
| [quest: Mortality Wanes]
| 
| **Razorfen Kraul**
|-
!colspan="3"|  Horde quests
|- class="alt"
| [quest: Tainted Brambleheart]
| 
| [Elder Rise](Elder Rise), [Thunder Bluff](Thunder Bluff)
|-
| [quest: Going, Going, Guano!]
| 
| [The Apothecarium](Apothecarium), [Undercity](Undercity)
|-
| [quest: An Unholy Alliance]
| [Small Scroll](Small Scroll), dropped by

| **Razorfen Kraul**
|-
| [quest: A Vengeful Fate]
| 
| [Thunder Bluff](Thunder Bluff), [Mulgore](Mulgore)
|-
!colspan="3"|  Neutral quests
|- class="alt"
| [quest: Blueleaf Tubers]
| 
| [Ratchet](Ratchet), [the Barrens](Barrens)
|-
| [quest: Willix the Importer]
| 
| **Razorfen Kraul**
|-
|}

## Patch changes
- (Patch 1.18.0|note=New area and boss added: Rotthorn. The caverns of Razorfen Kraul, wrapped in thorns, go deeper than presumed.)
-  **Patch 1.10.0 (2006-03-28):** Now capped at five players.
-  **Patch 1.3.0 (2005-03-07):** Now capped at ten players.
