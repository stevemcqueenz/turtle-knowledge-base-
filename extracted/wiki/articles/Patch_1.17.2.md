---
title: "Patch 1.17.2"
url: "https://turtle-wow.fandom.com/wiki/Patch_1.17.2"
source_type: "community wiki (turtle-wow.fandom.com), not an official source"
revision: "2026-06-03T05:29:19Z"
...
The Tower of Karazhan's haunted upper halls will become accessible to players starting this patch. This raid will feature the new Tier Set 3.5 and challenge 40 players to gameplay even more difficult than Naxxramas, allowing the truly determined to prove their strength against daunting foes.

Conquering the Tower of Karazhan will not only offer great loot and new Tier 3.5 sets, but players can also earn legendary titles:
...
To see the full changelog, please read [viewtopic.php?t=15157](https://web.archive.org/web/20260527132642/https://forum.turtlecraft.gg/viewtopic.php?t=15157)
...
Dungeon maps will be updated to include the new sections and bosses added by the dungeon extensions in the next patch, starting with the Wailing Caverns! 

### Alternative tier sets
- The itemization warrants its own changelog, which you can find here: [https://forum.turtlecraft.gg/viewtopic.php?t=10913](https://web.archive.org/web/20260515193715/https://forum.turtlecraft.gg/viewtopic.php?t=10913)
...
- New transport route: flight from Undercity to Steepcliff Port, and back.
- New transport route: flight from Ironforge to Gnomeregan Reclamation Facility, and back.
- ReReplaced the teleportation to Icepoint Rock with an actual boat route both ways
...
- Added a new option that will position the loot window at the cursor when enabled. This option can be found in Interface Options.
- Right-clicking an item in your bags while the auction house UI is open and the 'Auctions' tab is active will now put that item into the 'Auction Item' slot.
...
- The Looking for Turtles (LFT) bundled addon has been rewritten from the ground up. The client part of the addon handles the interface and user input, while the server handles the whole grouping logic. This move from a client-only approach provides a much more reliable way of finding dungeon groups.

As part of these changes, the 'Browse' tab has been removed and will be replaced with a better system in the upcoming patch.
- The 2FA randomized numpad is getting more comfortable in the upcoming patch. Yes, we felt the pain too

The donation rewards store has received several updates:
- Several categories now use a model instead of a static image to preview the offered item.
- Replaced custom entry images with the icon textures of the offered items.
- Fashion category now displays two entries instead of the usual eight. These entries are larger and display a dress-up model of your character with the entry item equipped so you can better preview the offered item.
- A new dropdown menu has been added which is shown when you click on the Fashion category. This dropdown menu can be used to filter entries by item type (Chest, Legs, Tabard etc.)
- Added a new confirmation pop-up dialog that is shown when you want to claim an item.
- Skins are now sorted based on your character's race and gender. Skins that are not usable by your race/gender combination are greyed out and can't be claimed.
- Glyphs are now sorted based on your character's class. Glyphs that are not usable by your class are greyed out and can't be claimed (implemented in case we introduce new glyphs for classes other than druid)
- Entry price will now be shown in red if you do not have the required balance to claim the offered item.
- Entries should now display in a more coherent fashion, for example mounts are now sorted based on mount type (horses, raptors, wolves etc.)
- General fixes and updates to visual appearance and performance.
...
- Level One Lunatic. During this challenge, you’ll earn exclusive titles to prove just how far a noob can go, all while staying level one!
  ...
  - The titles will stay with you, even if you eventually decide to level up

### General fixes and changes
- Armor values of trash mobs in Emerald Sanctum have been slightly reduced.
- Dreamstate in Emerald Sanctum is now an actual sleep and is now a Magic debuff, allowing it to be dispelled.
- The Green Dragonflight has gathered enough Arcanite Rods, and they have been removed from the required items to complete Emerald Sanctum raid attunement.
- Added rare mount drops to last bosses of Molten Core, Blackwing Lair and Temple of Ahn'Qiraj. Good luck!
- Hunters can now learn the new Rank 4 of the Thunderstomp pet ability from the gorillas of Tel'abim.
- Multiple new merchants moved into Alah'Thalas, offering wares previously unavailable in the High Elven capital, such as high-level trade goods and equipment.
- Removed outdoor requirement from Fishing Boat and Traveler's Tent crafting spells.
- Most custom beasts now have appropriate pet abilities when tamed which can be seen using Beast Lore.
- In addition to this, the previously unavailable ranks of Lightning Breath (Rank 1) and Charge (Rank 4) are now obtainable by players, we'll leave it up to you to discover the sources of these abilities.
...
- We're doubling the stable slots per character, allowing you to store 4 pets. The third slot costs gold, and the fourth requires a special glyph.
- Fixed Infernal Model disappearing when entering water.
...
- Hyjal's map is getting a major overhaul, allowing you to discover the mountain as you ascend, with a map that truly reflects the wonders and scars of this sacred peak!
...
- Fixed naming error in the Bael Hardul flight map.
- Flightmap now gives the proper names of some new zones when hovered over.
- Updated Visuals for the Worgen Caster.
- Updated visuals for Severance.
- Updated visuals for Wraith Blade.
- Minor fixes for the Undead Male Model.
- Fixed Revantusk Tabard visuals.
- Added new visual for Armored Black Deathcharger.
- Added /cower emote animation.
- The macro UI now features a searchbox that can be used to filter macro icons based on a keyword.
- Minor visual updates to the macro UI.
- Updated quest-giver interface: quests will now show a '?' or '!' icon instead of bullet points depending on whether the quest is available or in progress.
- Fixed a original Blizzard bug with Undead Female Tabard model clipping.
- Tallstrider mounts now have Backwalk, Jump and Mount Special animations.
- Assigned the correct instance and raid map types for Crescent Grove, Emerald Sanctum, and Stormwind Vault.
- Shademore Tavern in Gilneas will now correctly grant rested bonus.
- Several Visual Fixes & Updates to Naxxramas Weapon/Shield Models.
- Several Visual Fixes for Spells (Consecration, Earth/Water Shield, Holy Spells etc.)
- Beware the Blue Child, for its light may herald the dawn of a new age or the twilight of our own!

### Related dev blog posts
- [Dev Blog: Balance in 1.17.2](https://web.archive.org/web/20260504235439/https://forum.turtlecraft.gg/viewtopic.php?t=13840)
