---
title: "Windshear Crag"
url: "https://turtle-wow.fandom.com/wiki/Windshear_Crag"
source_type: "community wiki (turtle-wow.fandom.com), not an official source"
revision: "2025-12-03T16:05:08Z"
fetched: "2026-09-10T07:40:01Z"
is_turtle_content: false
patch: null
categories: ["Goblin territories", "Pages with broken file links", "Stonetalon Mountains subzones", "Valleys", "Venture Trading Company"]
---
# Windshear Crag

-Windshear Crag** once was a massive old-growth forest nestled in a valley in the [Stonetalon Mountains](Stonetalon Mountains). This cut in the mountains leads from the southern border of Ashenvale into the stormy canyons of the Stonetalons.

The Crag is now home to a massive logging expedition by the [Venture Company](Venture Company). The once green landscape is now a barren area of tree stumps, polluted waters, and creaking machinery. The entire logging operation was put in the hands of [Gerenzo Wrenchwhistle](Gerenzo Wrenchwhistle), a gnome who is now more machine than gnome. Using his mechanical expertise, he built the [Super Reaper 6000](Super Reaper 6000) which can reduce the most massive trees to sawdust. He watches over the crag from a platform built above the [Cragpool Lake](Cragpool Lake).

In the southeast corner of Windshear Crag lies the [Windshear Mine](Windshear Mine). Kobolds occupy the mine and are digging for [Smoky Iron Ingot](Smoky Iron Ingot)s. In the northeast corner is the [Talondeep Path](Talondeep Path). This cave stretches under the mountains and exits in [Ashenvale](Ashenvale). Nearby the entrance is [Braug Dimspirit](Braug Dimspirit), a [Tauren](Tauren) that administers the Test of Lore to members of the [Horde](Horde).

## Leaders of the Venture Co. in Windshear Crag
- [Gerenzo Wrenchwhistle](Gerenzo Wrenchwhistle)
- [Foreman Rigger](Foreman Rigger)
- [Taskmaster Whipfang](Taskmaster Whipfang)
