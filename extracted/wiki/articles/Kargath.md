---
title: "Kargath"
url: "https://turtle-wow.fandom.com/wiki/Kargath"
source_type: "community wiki (turtle-wow.fandom.com), not an official source"
revision: "2026-03-25T03:40:10Z"
fetched: "2026-09-10T07:39:04Z"
is_turtle_content: true
patch: "1.16.0"
categories: ["Badlands subzones", "Horde settlements", "Orc territories", "Turtle WoW content"]
---
# Kargath

| field | value |
|---|---|
| name | Kargath |
| ss | Kargath.jpg |
| faction | Horde |
| type | Outpost |
| race | Orc, Jungle troll, Tauren, Forsaken |
| leader | Warlord Goretooth |
| affiliation | Kargath Expeditionary Force, Orc clans |
| loc | Northwestern Badlands |
| facilities | yes |
| inn | yes |
| mailbox | yes |
| stables | yes |
| anvil | yes |
| travel | yes |
| boat | yes |
| flightpath | yes |
| status | Active |

''''

**Kargath** is a [Horde](Horde) in the western [Badlands](Badlands), named after the [Shattered Hand clan](Shattered Hand clan) chieftain [Kargath Bladefist](Kargath Bladefist). It was notable for having once been the only Horde outpost in [Khaz Modan](Khaz Modan), though it has since been joined by [Shatterblade Post](Shatterblade Post) in the [Grim Reaches](Grim Reaches).

Kargath's [flight path](flight path) is very important because it linked the northern and the southern areas of Horde flight paths on the [Eastern Kingdoms](Eastern Kingdoms). It links [Hammerfall](Hammerfall) and the [Undercity](Undercity) in the north to [Grom'gol](Grom'gol), [Booty Bay](Booty Bay), and [Stonard](Stonard) in the south.

## History
### World of Warcraft
On the orders of Warchief [Thrall](Thrall) himself, the [Kargath Expeditionary Force](Kargath Expeditionary Force) makes frequent excursions into the arid landscape of the [Badlands](Badlands). Exploration of the land revealed [Uldaman](Uldaman), an excavation site that seemed to be of high interest to the [dwarves](Dwarf), and Horde forces were sent to investigate the secrets the dwarves valued so highly. Yet the Expeditionary Force's primary goal was to secure a route through the [Searing Gorge](Searing Gorge) to [Blackrock Mountain](Blackrock Mountain) (so as to assault the Warchief of the [Dark Horde](Dark Horde), [Rend Blackhand](Rend Blackhand)). In doing so, the Horde inadvertently clashed with the [Dark Iron dwarves](Dark Iron clan) once again.

## Travel connections
### Flight
** [Undercity](Undercity), [Tirisfal Glades](Tirisfal Glades)**
** [Hammerfall](Hammerfall), [Arathi Highlands](Arathi Highlands)**
** [Flame Crest](Flame Crest), [Burning Steppes](Burning Steppes)**
** [Stonard](Stonard), [Swamp of Sorrows](Swamp of Sorrows)**
** [Grom'gol Base Camp](Grom'gol Base Camp), [Stranglethorn Vale](Stranglethorn Vale)**
** [Thorium Point](Thorium Point), [Searing Gorge](Searing Gorge)**
** [Booty Bay](Booty Bay), [Stranglethorn Vale](Stranglethorn Vale)**

### Zeppelin
** [Orgrimmar](Orgrimmar), [Durotar](Durotar)**

## NPCs
### Quest givers
- - - - - - - - - - ### Goods and services
- - - - - ### Additional characters
- - - - ## Patch changes
- (Patch 1.16.0|note=Zeppelin to Orgrimmar added.)
