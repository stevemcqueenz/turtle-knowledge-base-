...
## Start here: the class playbooks

**Players and bots should start with [`guide/`](guide/)**, the per-class playbooks for Turtle WoW 1.18.1, published on the site (<https://stevemcqueenz.github.io/turtle-knowledge-base-/>):

- `guide/classes/<class>/`: for each class, an `index.md` overview (spec viability for raid, dungeon, PvP, leveling and farming; 1.18.1 changes; races and professions), plus `leveling.md` (a validated talent order for every level from 10 to 60, the pull loop, gear and dungeons by bracket), one page per endgame spec (talent build with a working calculator link, stats and caps, single-target and AoE priority lists, cooldowns, gear, enchants, imbues/oils/poisons, consumables, raid notes and common mistakes), `pvp.md` and `sources.md` (the experts relied on, and the gaps).
- `guide/instances/`: 27 dungeon pages and 12 raid pages (Turtle custom and vanilla), each with boss-by-boss role duties, resistances and immunities.
- The bot playbooks in `structured/classes/<class>/*.yaml` match the guide. Each has a `guide:` key, talent points with `spell_id`s, a validated `leveling_order`, and rotation priority lists.

The guide is written mainly from a full read of the archived Discord for the 1.18.1 era (every message from 2025-10-03 on, plus highlights from 2024-11 to 2025-10), combined with the forum and wiki archive below. Every non-obvious claim is cited, and a QA pass checked each Discord citation against the claim it supports. How it was built is described in [`tools/discord-pipeline/`](tools/discord-pipeline/README.md).

## Layout

| directory | content |
|---|---|
| `guide/` | **the class playbooks and dungeon/raid pages** (see "Start here"); the source of the site's class, playbook, leveling, sources and instance pages |
| `research/discord-notes/` | cited reader notes, one file per Discord chunk (122 chunks, 10 channels) under fixed topic headings: the raw material the guide was written from. Verify a note's citation before reusing it |
| `tools/discord-pipeline/` | the chunker, the reader, writer, QA and instance briefs, and the validation tools (`talent_tool.py`, `check_notes.py`, `cite_pairs.py`, `collect_evidence.py`, ...) |
| `site/` | the React class-guide site (GitHub Pages); `site/scripts/build-data.py` builds its data from `guide/`, `structured/` and `synthesis/` |
