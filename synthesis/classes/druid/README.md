...
| `balance-ranged-dps.md` / `balance-ranged-dps.yaml` | Moonkin caster DPS, Eclipse mechanics, raid utility | **strong**: two long, detailed single-author write-ups (Tarsis pre-1.18.1, Scattyau pre-1.18.1-announced-pre-release) each corroborated by dozens of shorter replies; **medium** on the exact post-1.18.1 numeric outcome (contested, not log-verified) |
...
**Balance**: Balance of All Things reduced from 5 to 3 points and reworked (Wrath vs. Insect-Swarm'd target refunds 10/20/30% mana). The Starfire half changed **twice**: the **2026-01-26 revision** gave Starfire **−0.2/0.4/0.6 s reduced cast time** vs a Moonfire'd target [[d:druid#1465349903348924598]]; the **2026-03-19 revision replaced that with +3/6/9% crit**, which players read as a net nerf. Moonfury increased to 4/8/12% (2026-01-26 revision); Eclipse proc mechanism changed (2026-03-19 revision): "Damage from Wrath now has a 40% chance and Damage from Starfire now has a 60% chance to proc their beneficial effects" (superseding the 1.18.0-era 30%/50% chance mechanism documented in `synthesis/turtle-vs-vanilla-timeline.md`).
...
1. ~~**Full point-by-point 60 builds for every spec/role**~~ — **CLOSED for the main builds (Discord `staging/talents/`, 2026-09):** the `talents.turtlecraft.gg` links **decode**. The consensus Cat build is **11/35/5** [[d:druid#1413144334336790652]], the pinned Balance raid build is **35/0/13+3** [[d:druid#1356852934725472369]] (post-1.18.1 **37/0/14** [[d:druid#1485212531613241375]]), the Resto Tree build is **18/0/33** [[d:druid#1430728686411448410]] (Moonglow **21/0/30** [[d:druid#1421530945730646168]], Bapi **12/0/39** [[d:druid#1484966802101502214]]). The `spell_id` fields in the structured playbooks are now populated; only no-dedicated-optimal-bear-tree and the PvP builds remain partly unendorsed.
...
- ~~Fetch/decode the `talents.turtlecraft.gg` calculator links (the `?points=` query string) to turn every linked build in these playbooks into actual point lists~~ — **done** in the Discord merge (`staging/talents/`); the decoded allocations are in the structured playbooks and the synthesis Discord sections. Remaining: a dedicated post-1.18.1 bear tree and endorsed PvP point lists.
