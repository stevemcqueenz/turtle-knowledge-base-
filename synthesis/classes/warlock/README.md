...
**Discord (2026) — contested, not resolved.** The archived `#warlock` Discord shows a deliberate post-1.18.1 Soul-Link / Master-Demonologist tank package and a named AQ40 use: *"As demo you have Soul Link (20% dmg 'reduction'), Master Demonologist (10% dmg reduction) - gets you on regular tank level"* [[d:warlock#1486370117850763284]]; *"There's a big moment in AQ40 where it's actually necessary! Warlock tanks ftw"* [[d:warlock#1420543230188982313]]. This is **contested**: it is a niche/encounter role, not a general tank spec, and players still report the Voidwalker losing aggro even late in 1.18.1 ("still my voidwalker is losing aggro" at level 34 — Milordas, forum p=173046, 2026-03-29). Record it as contested; do not promote Warlock to a tank role.
...
3. **Full point-by-point 60 talent builds for Demonology and Destruction (Shadow/Fire) and for Demonology PvP**: every endgame build in those files is a `talents.turtlecraft.gg` or `talent-builder.haaxor1689.dev` link, not a decoded point list (only the primary Affliction leveling build in `leveling.md` is fully enumerated). — **CLOSED by Discord (2026)**: the pinned 1.18.1-announced-pre-release builds (Affliction 35/11/5, Demonology 0/34/17, Destruction 0/11/40; matching codes through 2026-04) and the PvP builds (Aff 35/5/11, SM/Ruin 30/0/21, Conflag/NF 19/0/32, SL 5/34/12) are now decoded with full point lists in the `*‑ranged-dps.yaml`/`*‑pvp.yaml` files and the synthesis files [[d:warlock#1484228321708478635]], [[d:warlock#1484228436888391690]], [[d:warlock#1484229282854342789]].
...
10. **Spell/talent IDs**: no source in this forum or the changelog gives numeric spell IDs for Warlock abilities, so every `spell_id` field in the structured YAMLs is `null`. — **CLOSED against the 1.18.1 tree**: Discord-decoded build allocations and `kb/structured/talents/warlock.yaml` supply the rank-specific `spell_id`s, now populated in the structured playbooks.
