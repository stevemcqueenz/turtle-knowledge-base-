    ...
    <ul className="grid gap-3 sm:grid-cols-2">
      {cooldowns.map((cd, i) => (
        <li key={i} className="card min-w-0 p-3.5 sm:p-4">
          <div className="text-[15px] font-bold"><CitedText text={describeValue(cd?.name) || 'Unnamed cooldown'} /></div>
          <div className="mt-1 text-sm leading-snug text-muted">
            <CitedText text={describeValue(cd?.use_when) || 'The sources do not say when to use it.'} />
          </div>
        </li>
      ))}
    </ul>
