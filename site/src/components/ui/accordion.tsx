...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex flex-1 items-center justify-between gap-3 py-3 text-left text-sm font-medium transition-colors hover:text-foreground [&[data-state=open]>svg]:rotate-180',
        className,
      )}
      {...props}
    >
