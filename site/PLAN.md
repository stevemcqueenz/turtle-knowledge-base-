...
`PlaybookYaml` fields the UI relies on (all optional, render only when present): `agreement` ("consensus"|"single"|"contested"), `patch_validity {written_for, valid_for_1181 (true|false|"unknown"|null), notes}`, `sources [{url, author, authority, date}]`, `talents {build_name, points [{tree, talent, rank, spell_id}], skip [], leveling_order [{level, tree, talent}], links_opaque []}` (a step's `level` is a number, or a string such as `"~14"` or `"10–33"` when the source gives only an approximate level or a bracket; `talents.leveling_order_approximate: true` flags those files), `stat_priority` (array of stat names, the curated sourced gearing order, present on 37 of 47 playbooks and deliberately absent on the rest because no source gives an order there; `stat_priority_note` carries the caps and contests — see AGENTS.md §2), `stat_weights` (object stat→weight or priority string; its keys are free-form source prose, not a stat list, and must not be rendered as a gearing order), `caps` (object), `rotation_single [{priority, action, condition}]`, `rotation_aoe [{targets_min, priority, action, condition}]`, `aoe_threshold`, `cooldowns [{name, use_when}]`, `resource_rules []`, `consumables []`, `mistakes_to_avoid []`. Values may be strings, lists or nested objects; the UI must tolerate any shape (render unknown shapes as a compact key/value list).
