...
`PlaybookYaml` fields the UI relies on (all optional, render only when present): `agreement` ("consensus"|"single"|"contested"), `patch_validity {written_for, valid_for_1181 (true|false|"unknown"|null), notes}`, `sources [{url, author, authority, date}]`, `talents {build_name, points [{tree, talent, rank, spell_id}], skip [], leveling_order [{level, tree, talent}], links_opaque []}` (a step's `level` is a number, sometimes written as a quoted number such as `"20"`, a bracket string such as `"10–33"` when the source gives a bracket, or null when the source gives the order but no level — null is why the renderer prints its "order but not the level" footnote), `stat_priority` (ordered array of canonical stat names drawn from a fixed vocabulary — caster specs use `spell-hit` and `spell-crit`, while `hit` and `crit` denote the physical melee/ranged stats, and hunters’ ranged AP is `ranged-attack-power` — the curated sourced gearing order, present on 36 of 47 playbooks and deliberately absent on the rest because no source gives an order there; every cap, target number and nuance lives in `stat_priority_note` instead — see AGENTS.md §2), `stat_weights` (object stat→weight or priority string; its keys are free-form source prose, not a stat list, and must not be rendered as a gearing order), `caps` (object), `rotation_single [{priority, action, condition}]`, `rotation_aoe [{targets_min, priority, action, condition}]`, `aoe_threshold`, `cooldowns [{name, use_when}]`, `resource_rules []`, `consumables []`, `mistakes_to_avoid []`. Values may be strings, lists or nested objects; the UI must tolerate any shape (render unknown shapes as a compact key/value list).
...
- **Glossary**: a side panel opened from the header; search; also used by the inline tooltip that explains the archive's own jargon (the Class Changes passes, the era labels, standing / contested / player claim, opaque build links) on its first mention in a guide body — click, tap or keyboard focus opens it; the archive's own labels (standing, contested, player claim) are matched only in their labelled form, not as ordinary English, and links, buttons, code, table cells and inline Markdown are left untouched.
...
```
Acceptance:
1. `build-data.py` produces 9 classes, 47 playbooks (matches `ls synthesis/classes/*/*-{tank,healer,melee-dps,ranged-dps,pvp}.md`), 9 leveling guides, 106 matrix rows, 323 glossary terms; every playbook has `sections.overview` and `sections.sources` non-null; a test asserts these counts.
...
## 11. Addendum: instance maps (2026-09)

Every dungeon and raid page opens with a map rendered from the Turtle WoW 1.18.1 client's minimap textures (`tools/maps/`, README there): one WebP per floor under `public/maps/<slug>/` (served as files, lazy-loaded, never inlined into JS), floor tabs, a click-to-enlarge lightbox with zoom and drag-to-pan, and numbered boss markers placed at the server's spawn coordinates and linked to the boss cards. Walkable areas the client never textured are shaded from the server navmesh; a map with no texture at all would be a navmesh "floor plan". `instances.json` pages gain an optional `map` (shape in `src/types.ts` `InstanceMap`; markers are matched against the page text by `build-data.py`, so guide edits never need the client); the index cards show a thumbnail. The single-file build leaves the maps out and says so.

## 12. Redesign v2: a WoW-native guide app (2026-09)

The v1 "field manual" read as machine-made (serif display type, parchment and brass, tracked-caps eyebrows, stat counters, hero copy about the site). v2 replaces the presentation layer only; the data pipeline, the JSON contract and the lazy chunks stay. Full description in `site/README.md` ("Design (v2)").

- **Stack.** shadcn/ui done properly: Radix primitives copied into `src/components/ui/` in shadcn style (Tailwind + `class-variance-authority` + `tailwind-merge`), `cmdk` for the ⌘K palette, `lucide-react` icons, Inter bundled from `@fontsource-variable/inter` (no runtime requests; the single-file build inlines it). Hash routing unchanged.
- **Shell.** Top bar (turtle logo `TurtleLogo`, search, theme, repository; an XP-bar reading-progress strip), collapsible left sidebar (classes → specs / leveling / PvP / professions / sources; dungeons & raids; professions; glossary), content column, right "On this page" list from `xl`; Sheet navigation and a sticky section dropdown on phones.
- **WoW-native language.** Official class colors per class page (`--cc`); ratings as item-quality badges (S legendary, A epic, B rare, C uncommon, none poor); in-game tooltips (navy panel, light border, gold title) for citations, glossary terms, ratings and talents; the talent grid as the in-game talent frame (square slots, corner rank green/gold/grey, gold prerequisite arrows, tree totals, talent tooltip); ability names as chat spell links.
- **New content surfaced.** `## Gameplay by level` is the leveling page's centrepiece (bracket segmented control, per-bracket blocks); `## How to play` is a numbered loop right after the build on every spec and PvP page; the bot panel has tabs for `play_loop` and `leveling_gameplay`.
- **New data and routes.** `build-data.py` writes the optional `src/data/professions.json` from `guide/professions.md` (GuideDoc + `recommendation`) and routes class professions pages to `#/class/<slug>/professions` and the overview to `#/professions` (links in every guide page follow). `#/glossary` is a page. The core module carries the professions headings for search; the overview is its own lazy chunk. `test_data.py` (`check_professions`) and `check-data.mjs` validate the file and its links; the smoke test covers every new route, the gameplay tabs, the How-to-play loop and the bot tabs (157 routes).
