...
Talent links (`structured/talent-links.jsonl`): `url, class, code, post_id, post_url, author, authority, date, era, topic_id, topic_title, context_before`. Code = three dash-separated tree segments, taken from the `talents.turtlecraft.gg/<class>/<code>` path segment on older links and from the `?points=` query value on newer ones; decode against Talent.dbc tree order (alphabet not confirmed; try the wowhead-style pairwise alphabet first). Coverage is uneven: the path form is indexed across the corpus, but the query form is indexed only for post 149715 (13 of the 372 distinct (post, `?points=` URL) pairs in the corpus) — the remaining 359 are a known, open gap.
