...
- Nothing here is invented. Every fact in `synthesis/`, `structured/` and `behavior/` carries a source (a post URL, a site/wiki page, or a Discord message citation); `python3 scrape/validate.py` proves each forum and Discord citation resolves to an archived record. When a document says "not found in sources", it means the whole 161,896-post corpus was searched.
...
- Discord is a fourth, player-side source tier: **Discord player / guide writer**, from the archived Turtle WoW Europe server (see `README.md`). It has **no green-name/staff signal** — rank a poster as guide writer only from content and reception (repeated well-received answers, pinned posts) or a matching name in `structured/forum/authors.json`, and say why; mark single-source claims as such.
- Discord citations are `[[d:<channel>#<message_id>]]` (e.g. `[[d:mage#818530154769940561]]`). `python3 scrape/validate.py` resolves each against `structured/discord/evidence-<channel>.jsonl`, which reproduces the cited message verbatim; the collector, `pipeline/collect_evidence.py`, lives outside this repo.
...
| a test checklist of known bugs | `structured/issues.yaml` (175 entries, `meta.status_values`; `synthesis/issues-summary.md` tier list) |
| things sources disagree on, with the field that settles each | `structured/contradictions.yaml` (184 entries, `meta.status_values`; 139 have `check_against_local_game_data`) |
...
| bot: twinking (bracket BiS, enchants, meta) | `structured/twinking.yaml` + `synthesis/twinking/README.md` (Discord `#twinking`) |
| bot: Level One Lunatic challenge | `structured/level-one-lunatic.yaml` + `synthesis/challenges/level-one-lunatic.md` (Discord `#level-one-lunatic`) |
| bot: addons, client mods, macros | `structured/addons.yaml`, `structured/macros.yaml` + `synthesis/guides/addons-and-macros.md` (Discord `#code-corner`) |
| bot: PvP systems (honour, War Mode, BGs, arena) | `structured/pvp.yaml` + `synthesis/pvp/matchups-cross-class.md` (Discord `#pvp-general`) |
| a decoded 1.18.1 talent tree or validated build | `structured/talents/<class>.yaml` (tabs, talents, rank `spell_id`s, `req`/`req_rank`) and `structured/talents/builds-<class>.yaml` (distinct builds; `allocation[]` has `talent_id` + rank `spell_id`; `builds_with_tree_drift` are pre-1.18.1 codes that fail current rules) |
| prove a Discord citation | `structured/discord/evidence-<channel>.jsonl` (`id`/`ts`/`author`/`content`); collector `pipeline/collect_evidence.py` (outside this repo) |
...
Issues (`structured/issues.yaml` -> `entries[]`, 175 entries; `meta.status_values`): `id (ISS-nnnn), title, symptom, reproduction, system, subsystem, entities[], entity_ids[], patch_reported, patch_fixed, era, status (open|confirmed|fixed|intended|unclear|not-root-caused|mitigated|resolved-later), status_source, severity (blocker|crash|exploit|major|minor|cosmetic), affects, reports[], notes`. `reports[]` is either the forum shape `{url,author,authority,date}` or the Discord shape `{cite,author,authority,date,era,agreement}` (no `url`; `cite` is `[[d:<channel>#<id>]]`; `era` and `agreement` as in `synthesis/CONVENTIONS.md`).
Contradictions (`structured/contradictions.yaml` -> `entries[]`, 184 entries; `meta.status_values`): `id (CON-nnnn), system, topic, side_a/side_b(/side_c) {claim, sources[]}, what_would_settle_it, assessment, status, check_against_local_game_data (bool)`. A source is normally `{author,authority,date,url}` and may carry `cite` (plus `era`, `agreement`) instead of `url` for a Discord message. Some entries add an optional top-level `discord:` block `{note, sources:[{channel,id,era,agreement,cite}]}` for two-sided or Discord-only evidence.
...
4. Then `issues.yaml` in the `synthesis/issues-summary.md` tier order (there is no `meta.priority_order`); status `intended` entries describe behavior the emulator should reproduce, not fix.
...
7. For talents: **decoding is done.** The 1.18.1 trees are in `structured/talents/<class>.yaml` (tabs, talents, rank `spell_id`s, `req`/`req_rank`) and `structured/talents/builds-<class>.yaml` holds 1,719 distinct 51-point builds decoded from forum and Discord build codes, each `allocation` carrying `talent_id` and rank `spell_id`; codes that no longer satisfy 1.18.1 are kept and flagged in `builds_with_tree_drift`. The playbooks' `talents.points` are filled from these (677 of 720 have a `spell_id`; the rest are talents with no name match in the 1.18.1 tree).
