...
Class playbook YAML (`structured/classes/<class>/<spec>-<role>.yaml`): `class, spec, role, sources[{url,author,authority,date}], agreement (consensus|single|contested), patch_validity {written_for, valid_for_1181, notes}, talents {build_name, points[{tree,talent,rank,spell_id}], skip[], leveling_order[{level,tree,talent}], links_opaque[]}, stat_weights{}, caps{}, rotation_single[{priority,action,condition}], rotation_aoe[{targets_min,priority,action,condition}], aoe_threshold, cooldowns[{name,use_when}], resource_rules[], consumables[], mistakes_to_avoid[]`. `spell_id` is null everywhere (no source gives IDs): fill it from Spell.dbc by name. Two fields are easy to confuse: `stat_weights` is free-form prose keyed by whatever the sources talked about (its keys are not a stat list and must not be read as a gearing order), while `stat_priority` is the curated, sourced order: an ordered array of canonical stat names drawn from a fixed vocabulary (stamina, strength, agility, intellect, spirit, armor, attack-power, ranged-attack-power, crit, spell-crit, hit, spell-hit, haste, spell-power, healing-power, mp5, weapon-skill, defense, dodge, parry, block, block-value, spell-penetration, resistance) so the arrays join across specs, with every cap, target number and nuance carried in `stat_priority_note` instead. Present on 37 of 47 playbooks, deliberately **absent** on the rest because no source gives an order there. The `order` lists inside each leveling YAML's `stat_priority` use the same vocabulary. `talents.leveling_order` levels may be a string when the source itself gives a bracket (`"10–14"`, `"10–33"`); those are ordinary source-given values and are not flagged. `level: null` means the source gives the order but no level at all. `leveling_order_approximate: true` marks only files whose levels are derived rather than the source's — currently druid/balance-ranged-dps.yaml and shaman/enhancement-melee-dps.yaml. And `leveling_order_source` is the prose sentence saying which post the order came from and whether the levels are the source's or derived from it.
Leveling YAML (`structured/classes/<class>/leveling.yaml`, one per class): `class, source_file, patch_validity{}, spec_verdict{favored{spec,build,agreement,why,sources[]}, alternatives[], avoid[]}, opener[{step,spec,action,condition,sources[]}], priority[{priority,action,condition,agreement,sources[]}], mob_selection{rule,agreement,sources[]}, weapon_rule{...}, stat_priority[{spec,order[],notes,agreement,sources[]}] or null, gaps[]`. Citations use the same `{url,author,authority,date}` shape as the playbooks. Not yet emitted by `site/scripts/build-data.py`.
Talent-order tables in `synthesis/classes/<class>/leveling.md` all use one shape — `| Levels | Talent | Note |`, one row per step, points folded into the Talent cell, tree in parentheses where the route mixes trees. Nothing else in those files uses that header, but the header alone does not identify a build: the owning `###`/`####` heading does, and one file may hold several mutually exclusive tables — mage/leveling.md has four (Frost single-target, Frost AoE, Fire, Arcane), rogue/leveling.md three (main line, past-44 continuation, the SvenS2 dungeon order), paladin/leveling.md two (Protection, Retribution), shaman/leveling.md two (Enhancement, Restoration). Keep new tables in that shape rather than adding a parser variant.
...
Talent links (`structured/talent-links.jsonl`): `url, class, code, post_id, post_url, author, authority, date, era, topic_id, topic_title, context_before`. Code = three dash-separated tree segments, taken from the `talents.turtlecraft.gg/<class>/<code>` path segment on older links and from the `?points=` query value on newer ones; decode against Talent.dbc tree order (alphabet not confirmed; try the wowhead-style pairwise alphabet first). Coverage is partial, and the counts below are measured (2026-09-12), not guaranteed: 208 rows index 140 of the corpus's 163 distinct path-form build URLs (23 missing), and only 13 of its ~371 distinct (post, `?points=` URL) query-form pairs, all from post 149715. Both shortfalls are open gaps — see `structured/talent-links.README.md`.
