...
Talent links (`structured/talent-links.jsonl`): `url, class, code, post_id, post_url, author, authority, date, era, topic_id, topic_title, context_before`. Code = up to three dash-separated tree segments (the calculator omits trailing empty trees: of 195 path-form codes, 121 have three segments, 54 two and 20 one — pad, do not assume three), taken from the `talents.turtlecraft.gg/<class>/<code>` path segment; decode against Talent.dbc tree order (alphabet not confirmed; try the wowhead-style pairwise alphabet first).
