...
Talent-order tables in `synthesis/classes/<class>/leveling.md` all use one shape — `| Levels | Talent | Note |`, one row per step, points folded into the Talent cell, tree in parentheses where the route mixes trees. Nothing else in those files uses that header, but the header alone does not identify a build: the owning `###`/`####` heading does, and one file may hold several mutually exclusive tables — mage/leveling.md has four (Frost single-target, Frost AoE, Fire, Arcane), rogue/leveling.md three (main line, past-44 continuation, the SvenS2 dungeon order), paladin/leveling.md two (Protection, Retribution). shaman/leveling.md's single table ends with two `51–60 | Restoration alternative: …` rows, so a parser keyed on the header alone would emit Restoration talents as part of the Enhancement leveling order. Keep new tables in that shape rather than adding a parser variant.
