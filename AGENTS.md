...
Talent-order tables in `synthesis/classes/<class>/leveling.md` all use one shape — `| Levels | Talent | Note |`, one row per step, points folded into the Talent cell, tree in parentheses where the route mixes trees. Nothing else in those files uses that header, so it is safe to parse on. Keep new tables in that shape rather than adding a parser variant.
